var squareObjects = new Array();
	...
	function highlightSquare(e,inputObj){
		...
		if(!inputObj)checkerrors = true;	
		if(!inputObj)inputObj = this;	
		if(inputObj.style.backgroundColor)return;
		if(gameFinished)return;
          	inputObj.oldcls = inputObj.className;
		inputObj.className='sudokuSquareHighlighted';
		if(higlightedCell && higlightedCell!=inputObj)higlightedCell.className=higlightedCell.oldcls || 'sudokuSquare';
	...
	document.getElementById('hintDiv').style.display='none';
		...
		// checking for errors for whole game board
		checkBoardForErrors();
	...
	function checkRowColForErrors(row, col) {
  		var nums = [];
    		var error = false;
		var varient = row == undefined ? row : col;
		var constant = row !== undefined ? row : col;

      	for (varient = 0; varient < 9; varient++) {
			var spans = document.getElementById('square_'+constant+'_'+varient).getElementsByTagName('SPAN');
			if (row == undefined) {
				spans = document.getElementById('square_'+varient+'_'+constant).getElementsByTagName('SPAN');
			}
		    	var curVal = '';

			if (spans[0].style.display == '') {
				curVal = spans[0].innerHTML;
			}
			else {
				curVal = spans[1].innerHTML;
			}
		    	if (curVal) {
		...
		if (error){
      		for (varient = 0; varient < 9; varient++){
			  var box = document.getElementById('square_'+constant+'_'+varient);
			  if (row == undefined) {
				box = document.getElementById('square_'+varient+'_'+constant);
			  } 
			  if (box.className !== 'sudokuSquareHighlighted') {box.className='error';}
			}
		}
	}

	function checkBoardForErrors(){
    		clearBoardErrors(); // clearing errors
		for (var i = 0; i < 9; i++)
    		{
    			checkRowColForErrors(i, undefined); // for rows
		    	checkRowColForErrors(undefined, i); // for columns
		      checkBoxForErrors(i);   		// for 3 x 3 box
		}
  	}

	function checkBoxForErrors(num) {
  		var nums = [];
    		var error = false;
		var boxX = (num % 3) * 3;
    		var boxY = (Math.floor(num / 3)) * 3;

      	for (var row = boxY; row < boxY + 3; row++) {
			for (var col = boxX; col < boxX + 3; col++) {

			var spans = document.getElementById('square_'+row+'_'+col).getElementsByTagName('SPAN');
		    	var curVal = '';
			if (spans[0].style.display == '') {
				curVal = spans[0].innerHTML;
			}
	    ...
	    if (error){
			for (var row = boxY; row < boxY + 3; row++){
	        		for (var col = boxX; col < boxX + 3; col++){
					var box = document.getElementById('square_'+row+'_'+col);
      				if (box.className !== 'sudokuSquareHighlighted') {box.className='error';}
			    }
		   	}
		}
	...
	function clearBoardErrors()
  	{
  		for (var row = 0; row < 9; row++){
				...
				var box = document.getElementById('square_'+row+'_'+col);
      			if (box.className !== 'sudokuSquareHighlighted') {box.className='sudokuSquare';}
      			if (box.className !== 'sudokuSquareHighlighted') {box.oldcls='sudokuSquare';} 
			}
		}
  	}
